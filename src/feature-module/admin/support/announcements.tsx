...
import { fetchAnnoucement } from '../../../APICalls';
import moment from 'moment';
import { Announcement } from '../../../GlobleType';

const Announcements = () => {
  const [selectedValue, setSelectedValue] = useState(null);
  const [selectedData, setSelectedData] = useState<Announcement>();
  ...
  const fetchData = async () => {
    try {
      const data = await fetchAnnoucement();
      if (data) {
        const formattedData = data.map((e: Announcement, index: number) => ({
          ...e,
          id: index + 1,
  ...
  const sortData = (categories: any, order: string) => {
    return categories.sort((a: Announcement, b: Announcement) => {
      if (order === 'A - Z') {
  ...
  return (
    <>
      ...
      <AnnouncementsModal values={selectedData} updateCus={() => fetchData()} />
    </>
  );
};
